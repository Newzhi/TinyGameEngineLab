...
| **本文档** | 模板缓冲、`glStencilFunc` / `glStencilOp` / mask、物体轮廓与其它用法思路 |
| 后续 | 混合、面剔除、帧缓冲…… |
...
**已实现最简 Demo**（`main.cpp` + `shaders/stencilColor.*`）：

1. 中央矩形写入模板值 `1`（关颜色写入，只打标签）  
2. `GL_NOTEQUAL, 1` 画全屏 → **画面 B（蓝）**（模板不通过区）  
3. `GL_EQUAL, 1` 画全屏 → **画面 A（橙）**（模板通过区）  

运行后应看到：屏幕中间一块橙色，周围蓝色。改 `maskQuad` 顶点即可改「通过」区域形状。

若要做官网「背包/箱子描边」，可在此基础上再加：`ALWAYS+REPLACE` 画物体 → `NOTEQUAL` 画放大单色版。
...
2. 为什么默认 `glStencilOp(GL_KEEP,GL_KEEP,GL_KEEP)` 时，开了模板测试也「写不进标签」？  
3. 轮廓算法里，为什么第二遍要用 `GL_NOTEQUAL` 而不是 `GL_EQUAL`？  
4. 画地板时为什么常先 `glStencilMask(0x00)`？  
5. （思考）若希望两个重叠选中单位**各自**有完整描边而不是粘成一圈，流程要怎么改？  
6. （可选）对照官网仓库 `4.advanced_opengl/2.stencil_testing` 源码，把步骤编号写在旁注里。
